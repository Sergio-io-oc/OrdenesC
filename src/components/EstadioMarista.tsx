...
export const EstadioMaristaControl: React.FC = () => {
  ...
  // Carga inicial al abrir en PC o Móvil
  useEffect(() => {
    loadTripsFromSheet();
  }, [loadTripsFromSheet]);
  ...
  return (
    <div className="space-y-8">
      {/* Banner / Title Header */}
      <div className="bg-gradient-to-r from-blue-700 via-indigo-700 to-emerald-700 rounded-3xl p-6 sm:p-8 text-white shadow-lg relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/20 backdrop-blur-md text-xs font-semibold uppercase tracking-wider mb-3">
              <MapPin className="w-3.5 h-3.5" /> Estadio Marista
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold tracking-tight">
              Control de Llegadas y Regresos
            </h2>
            <p className="text-blue-100 text-sm sm:text-base mt-1 max-w-xl">
              Registro en tiempo real de los Buses y Vans que ingresan al Estadio Marista y posterior confirmación de su regreso al colegio.
            </p>
          </div>

          <div className="flex items-center gap-4 bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20">
            <div className="text-center px-3 border-r border-white/20">
              <span className="block text-2xl font-black">{activeVehicles.length}</span>
              <span className="text-[11px] text-blue-100 uppercase tracking-wider font-medium">En Estadio</span>
            </div>
            <div className="text-center px-3">
              <span className="block text-2xl font-black">{trips.filter(t => t.status === 'returned').length}</span>
              <span className="text-[11px] text-blue-100 uppercase tracking-wider font-medium">Regresaron</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="px-3 py-1.5 bg-emerald-50 border border-emerald-200/80 text-emerald-800 rounded-xl text-xs font-bold flex items-center gap-1.5 shadow-2xs">
              ...
              <span>{isSyncing ? 'Sincronizando...' : (syncStatus || 'Sincronizado con Google Sheets')}</span>
            </div>

            {/* Actualizar de Google Sheets */}
            <button
              onClick={loadTripsFromSheet}
              disabled={isSyncing}
              ...
              title="Cargar y actualizar registros desde la planilla Google Sheets"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
              <span>Actualizar de Google Sheets</span>
            </button>

            {/* Ver en Google Sheets */}
            <a
              href="https://docs.google.com/spreadsheets/d/10fBagqN02_xqo_wvrPx5WlqzJOpBF3YZ6sTeenRlfjA/edit?gid=0#gid=0"
              target="_blank"
            ...
            >
              <ExternalLink className="w-3.5 h-3.5 text-emerald-600" /> Ver Sheet
            </a>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200/80 text-slate-500 font-bold uppercase tracking-wider">
                ...
                <th className="p-3.5 rounded-r-xl text-right">Acción</th>
              </tr>
            </thead>
