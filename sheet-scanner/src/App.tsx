...
export default function App() {
  ...
  const [paperData, setPaperData] = useState<PaperDataRow[]>([]);
  ...
  const processPaperSheet = (rows: any[][]) => {
    if (!rows || rows.length < 4) return;
    const paperRows: PaperDataRow[] = [];
    ...
    for (let i = 3; i < rows.length; i++) {
      ...
      if (!dateStr || dateStr.toUpperCase().includes('TOTAL') || dateStr.toUpperCase().includes('GLOBAL')) continue;
      
      const parseVal = (val: any) => {
        if (typeof val === 'number') return val;
        if (typeof val === 'string') {
          const valStr = val.replace(/\$|\./g, '').trim();
          return valStr && valStr !== '-' ? parseInt(valStr, 10) : 0;
        }
        return 0;
      };
      ...
      paperRows.push({
        date: dateStr,
        qtyCarta: parseVal(row[1]),
        valCarta: parseVal(row[2]),
        qtyOficio: parseVal(row[3]),
        valOficio: parseVal(row[4]),
        valTotal: parseVal(row[5]),
      });
    }
    setPaperData(paperRows);
  };
  ...
  }, [selectedSheet, dashboardType, fetchPaperData]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#ebf4f5] to-[#f5f7fa] font-sans text-slate-900 pb-12">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-8">
                ...
                <AreaChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                  <defs>
                    <linearGradient id="colorTotal" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor={dashboardType === 'movilizacion' ? "#3b82f6" : "#10b981"} stopOpacity={0.3}/>
                      <stop offset="95%" stopColor={dashboardType === 'movilizacion' ? "#3b82f6" : "#10b981"} stopOpacity={0}/>
                    </linearGradient>
                  </defs>
                  ...
                  <Area 
                    ...
                    dataKey="total" 
                    name="Gasto Total"
                    stroke={dashboardType === 'movilizacion' ? "#3b82f6" : "#10b981"} 
                    strokeWidth={3}
                    fillOpacity={1} 
                    fill="url(#colorTotal)" 
                  />
        ...
        {dashboardType === 'impresiones' && (
          ...
          >
            <h3 className="text-lg font-bold text-slate-800 mb-6">
              Compra de Resmas (Carta y Oficio)
            </h3>
            {selectedSheet.includes('2025') ? (
              <div className="h-[300px] w-full flex items-center justify-center border-2 border-dashed border-slate-100 rounded-2xl">
                <p className="text-slate-400 font-medium">Sin datos de compras para este período</p>
              </div>
            ) : paperData.length > 0 ? (
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={paperData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                        ...
                        <YAxis 
                          ...
                          tickLine={false} 
                          tick={{ fontSize: 12, fill: '#64748b' }}
                          tickFormatter={(val) => val >= 1000000 ? `$${(val / 1000000).toFixed(1).replace('.', ',')}M` : `$${(val / 1000).toFixed(0)}k`}
                          dx={-10}
                        />
                        <Tooltip 
                          contentStyle={{ borderRadius: '12px', border: '1px solid #f1f5f9', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
