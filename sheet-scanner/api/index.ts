...
app.get('/api/paper', async (req, res) => {
  try {
    const url = 'https://docs.google.com/spreadsheets/d/1_I1Qs4Fc-Pm__rm7TcpsA9d1mOwVUXIH/export?format=xlsx';
    
    const response = await fetch(url);
    if (!response.ok) throw new Error('Failed to fetch paper data from Google Sheets');
